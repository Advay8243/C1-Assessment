...
Raw landing only. NULL names/categories, duplicate product_id rows,
cost>price, and negative stock are kept for Silver. No quality checks here.
"""
...
def ingest_products(spark, landing_dir: str, batch_id: str, ingest_ts):
    return ingest_table(spark, "products", landing_dir, batch_id, ingest_ts)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Ingest products.csv into bronze.products")
...
def main() -> None:
    ...
    result = ingest_products(spark, landing_dir, batch_id, ingest_ts)
    print(result)
