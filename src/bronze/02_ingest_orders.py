...
Raw landing only. NULL FKs, orphan FKs, duplicate order_id rows, and
broken amounts/status dates are kept for Silver. No quality checks here.
"""
...
def ingest_orders(spark, landing_dir: str, batch_id: str, ingest_ts):
    return ingest_table(spark, "orders", landing_dir, batch_id, ingest_ts)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Ingest orders.csv into bronze.orders")
...
def main() -> None:
    ...
    result = ingest_orders(spark, landing_dir, batch_id, ingest_ts)
    print(result)
